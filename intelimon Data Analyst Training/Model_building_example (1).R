...
pred <-read.csv("SCSRF_pred (1).csv")  #use the file path to physical data we are trying to predict



vars <-read.csv("SCSRF_var (1).csv")#use the file path to point cloud metrics
...
regfit = regsubsets(train_pred$MFBD #adjust this
                    ~., train_vars,  nvmax = 4 #adjust this
                    , method = "seqrep"# adjust this
                    , really.big = T) 
...
coef(regfit,8)# change the number to the best model then copy the predictors 
...
lm1 <- lm(formula = train_pred$MFBD~  
)
...
data<-data.frame(x=lm_out, y=train_pred$PPBA)
...
errors<- test_out - test_pred$HWPBA
mae <- mean(abs(errors))
mse <- mean(errors^2)
rmse <- sqrt(mse)
rmse
...
data2<- data.frame(x=test_out, y=test_pred$HWPBA)
...
saveRDS(lm1, file.path(wd, "MFBD.rda") )
